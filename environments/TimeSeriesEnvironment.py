...
from utils.GeneralUtils import normData
import pandas as pd
...
class TimeSeriesEnvironment(gym.Env):
    ...
    def createDataframes(self, configuration):
        ...
        self.baseSeed = configuration["base_seed"]
        ...
        nonRLComparisonStrategies = {
            ...
            "All Stocks Buy-and-Hold": np.array(
    ...
    def partitionData(self, data, configuration):
        ...
        datasetsAndDetails = datasets.update(details)
        self.datasetsAndDetails = datasetsAndDetails
    ...
    def setOutofSampleData(self, dataType, useNoiseEval=True, epoch=0):
        marketData = self.datasetsAndDetails[f"{dataType}_data"]
        normalisedData = {}
        dataShape = marketData.shape
        PRICE_DATA = {}
    ...
    def pushTrainingWindow(self, episode, evalType, epoch):
        """
        ...
        self.normDataTensors = {
            ...
            for key, value in normalisedData.items()
        }
